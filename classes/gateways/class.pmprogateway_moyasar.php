<?php
	...
	add_action('init', array('PMProGateway_moyasar', 'init'));

	/**
	 * PMProGateway_gatewayname Class
	 *
	 * Handles moyasar integration.
	 *
	 */
	class PMProGateway_moyasar extends PMProGateway
	{
		...
		{
			//make sure moyasar is a gateway option
			add_filter('pmpro_gateways', array('PMProGateway_moyasar', 'pmpro_gateways'));

			//add fields to payment settings
			add_filter('pmpro_payment_options', array('PMProGateway_moyasar', 'pmpro_payment_options'));
			add_filter('pmpro_payment_option_fields', array('PMProGateway_moyasar', 'pmpro_payment_option_fields'), 10, 2);

			//add some fields to edit user page (Updates)
			add_action('pmpro_after_membership_level_profile_fields', array('PMProGateway_moyasar', 'user_profile_fields'));
			add_action('profile_update', array('PMProGateway_moyasar', 'user_profile_fields_save'));

			//updates cron
			add_action('pmpro_activation', array('PMProGateway_moyasar', 'pmpro_activation'));
			add_action('pmpro_deactivation', array('PMProGateway_moyasar', 'pmpro_deactivation'));
			add_action('pmpro_cron_moyasar_subscription_updates', array('PMProGateway_moyasar', 'pmpro_cron_moyasar_subscription_updates'));

			//code to add at checkout if moyasar is the current gateway
			$gateway = pmpro_getOption("gateway");
			if($gateway == "moyasar")
			{
				add_action('pmpro_checkout_preheader', array('PMProGateway_moyasar', 'pmpro_checkout_preheader'));
				add_filter('pmpro_checkout_order', array('PMProGateway_moyasar', 'pmpro_checkout_order'));
				add_filter('pmpro_include_billing_address_fields', array('PMProGateway_moyasar', 'pmpro_include_billing_address_fields'));
				add_filter('pmpro_include_cardtype_field', array('PMProGateway_moyasar', 'pmpro_include_billing_address_fields'));
				add_filter('pmpro_include_payment_information_fields', array('PMProGateway_moyasar', 'pmpro_include_payment_information_fields'));
			}
		}

		/**
		 * Make sure moyasar is in the gateways list
		 *
		 * @since 1.8
		 */
		static function pmpro_gateways($gateways)
		{
			if(empty($gateways['moyasar']))
				$gateways['moyasar'] = __('moyasar', 'pmpro');

			return $gateways;
		}

		/**
		 * Get a list of payment options that the moyasar gateway needs/supports.
		 *
		 * @since 1.8
		 */
		...
		{
			//get moyasar options
			$moyasar_options = PMProGateway_moyasar::getGatewayOptions();

			//merge with others.
			$options = array_merge($moyasar_options, $options);

			return $options;
		}

		/**
		 * Display fields for moyasar options.
		 *
		 * @since 1.8
		 */
		...
		{
		?>
		<tr class="pmpro_settings_divider gateway gateway_moyasar" <?php if($gateway != "moyasar") { ?>style="display: none;"<?php } ?>>
			<td colspan="2">
				<?php _e('moyasar Settings', 'pmpro'); ?>
			</td>
		</tr>
		<tr class="gateway gateway_moyasar" <?php if($gateway != "moyasar") { ?>style="display: none;"<?php } ?>>
			<?php // moyasar custom pamyment settings here ?>
		</tr>
		<?php
		}
		...
		{
			wp_schedule_event(time(), 'daily', 'pmpro_cron_moyasar_subscription_updates');
		}
		...
		{
			wp_clear_scheduled_hook('pmpro_cron_moyasar_subscription_updates');
		}
		 ...
		 * @since 1.8
		 */
		static function pmpro_cron_moyasar_subscription_updates()
		{
		}
